...
<script async src="https://www.googletagmanager.com/gtag/js?id=AW-18373055367"></script>
<script>
...
<link rel="canonical" href="https://deviceglowhub.com/{geo}/{slug}/landing.html">
</head>
<body>
...
def transform_landing(html: str, offer_id: str, cfg: dict) -> str:
    ...
    html = html.replace("Sweevo", "Vacuza")
    html = html.replace("sweevo", "vacuza")
    html = html.replace("gbyteit.com", "deviceglowhub.com")
    html = html.replace("info@gbyteit.com", "info@deviceglowhub.com")
    html = html.replace("SUPERADS INTERNATIONAL TECHNOLOGY LIMITED", "COD KINGDOM LLC")
    html = html.replace("RM 1502-A EASEY COMM BLDG 253-261 HENNESSY RD", "111 NE 1st St, 8th Floor 88730")
    html = html.replace("Wan Chai, Hong Kong", "Miami, FL 33132, United States")
    html = html.replace('site-logo__text-primary">gby</span><span class="site-logo__text-accent">teit', 'site-logo__text-primary">deviceglowhub</span><span class="site-logo__text-accent">.com')
    html = html.replace('aria-label="gbyteit home"', 'aria-label="deviceglowhub.com home"')
    html = html.replace(f'href="/{geo}/"', 'href="/"')
    html = html.replace('href="https://deviceglowhub.com/"', 'href="/"')
...
def transform_thank_you(html: str, offer_id: str, cfg: dict) -> str:
    ...
    html = html.replace("Sweevo™", "Vacuza™")
    html = html.replace("Sweevo", "Vacuza")
    html = html.replace("gbyteit", "deviceglowhub")
    html = html.replace("info@gbyteit.com", "info@deviceglowhub.com")
    html = html.replace("SUPERADS INTERNATIONAL TECHNOLOGY LIMITED", "COD KINGDOM LLC")
    html = html.replace("RM 1502-A EASEY COMM BLDG 253-261 HENNESSY RD — Wan Chai, Hong Kong", "111 NE 1st St, 8th Floor 88730, Miami, FL 33132, United States")
    html = html.replace('site-logo__text-primary">gby</span><span class="site-logo__text-accent">teit', 'site-logo__text-primary">deviceglowhub</span><span class="site-logo__text-accent">.com')
    html = html.replace(f'href="/{geo}/"', 'href="/"')
